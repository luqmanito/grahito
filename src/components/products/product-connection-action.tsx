...
export function ProductConnectionAction({ authenticated, available, connected }: { authenticated: boolean; available: boolean; connected: boolean }) {
  ...
  if (!available) {
    return <span className="inline-flex h-12 items-center rounded-full bg-paper px-5 text-sm font-semibold text-muted">{dictionary.products.unavailable}</span>;
  }

  if (state.connected) {
    return <ButtonLink href="/account"><CheckCircle2 className="size-4" />{dictionary.products.connected}</ButtonLink>;
  }

  if (!authenticated) {
    return <ButtonLink href="/register"><UserPlus className="size-4" />{dictionary.products.registerToConnect}</ButtonLink>;
  }
  ...
  return (
    <form action={action}>
      <Button type="submit" disabled={pending}>
        ...
        {pending ? dictionary.products.connecting : dictionary.products.connect}
      </Button>
