...
export function LoginForm() {
  ...
  async function onSubmit(values: LoginValues) { setError(""); try { const { error } = await createClient().auth.signInWithPassword(values); if (error) return setError(authErrorMessage(error.message)); const requestedNext = params.get("next"); const safeNext = requestedNext?.startsWith("/") && !requestedNext.startsWith("//") ? requestedNext : "/account"; router.push(safeNext); router.refresh(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Terjadi kendala. Silakan coba lagi."); } }
