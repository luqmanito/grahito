      ...
      {open && (
        <div id="mobile-navigation" className="absolute inset-x-4 top-20 rounded-3xl border border-line bg-white p-4 shadow-2xl">
          <nav className="flex flex-col" aria-label="Navigasi seluler">
            {[{href:"/products",label:"Produk"},{href:"/#tentang",label:"Tentang"},{href:"/support",label:"Bantuan"},{href:"/login",label:"Masuk"}].map((item) => <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="rounded-xl px-4 py-3 text-sm font-medium hover:bg-paper">{item.label}</Link>)}
