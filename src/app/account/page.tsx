...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ password?: string }> }) {
  ...
  const [{ data: profile }, { data: userProducts }, { data: productDevices }] = await Promise.all([
    ...
    supabase.from("product_devices").select("id, device_name, platform, last_seen_at, created_at, products!inner(slug)").eq("user_id", user.id).eq("active", true).order("created_at", { ascending: true }),
  ]);
  ...
  return (
    <section className="bg-paper py-12 sm:py-16"><Container className="max-w-5xl">
      ...
      <DeviceManagement connected={kalkulatorConnected} devices={devices} />
    </Container></section>
  );
}
