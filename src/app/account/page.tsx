...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ password?: string }> }) {
  ...
  const [{ data: profile }, { data: userProducts }] = await Promise.all([
  ...
  return (
    <section className="bg-paper py-12 sm:py-16"><Container className="max-w-5xl">
      ...
      <div className="mt-5 rounded-3xl border border-line bg-white p-6 sm:p-8"><div className="mb-6 flex items-center gap-3"><Package className="size-5 text-muted" /><h2 className="text-lg font-semibold text-ink">Produk terhubung</h2></div>{products.length === 0 ? <EmptyState title="Belum ada produk terhubung" description="Belum ada produk yang terhubung ke akun ini." /> : <div className="space-y-3">{products.map((entry) => { const linkedProduct = Array.isArray(entry.products) ? entry.products[0] : entry.products; return <div key={entry.id} className="flex items-center justify-between rounded-2xl border border-line p-4"><div><p className="font-semibold text-ink">{linkedProduct?.name || "Produk Grahito Labs"}</p><p className="mt-1 text-xs text-muted">Paket {entry.plan}</p></div><span className="rounded-full bg-lime-soft px-3 py-1 text-xs font-semibold text-lime-dark">{entry.status}</span></div>; })}</div>}</div>
    </Container></section>
  );
}
